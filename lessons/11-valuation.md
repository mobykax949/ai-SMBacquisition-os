...
The EPIC valuation method, end to end. It chooses the right profit metric (SDE for owner-operated, EBITDA for professionally managed), computes trailing-twelve-month profit from the actual statements, then runs the audit that matters most: for every add-back the seller claims, it subtracts the true cost to replace the owner's role — the discipline that protects you from overpaying by hundreds of thousands (a 20–40% overpay is this system's working estimate of the typical damage, not a published statistic). It anchors the multiple on Frasier's landscape — owner-operated 2.4x SDE, professionally managed 4.0x EBITDA, private equity 10.8x, public markets 18.7x (Frasier's anchors; the in-between bands are this system's working bands) — and ends with your three prices: walk-away, realistic, dream. A tax-return sanity test confirms the P&L matches Schedule C or the corporate return before you trust any of it. One boundary: this is analysis, not accounting — before money moves, a CPA verifies the financials and a lawyer papers the deal.
