...
3. **Audit the add-backs.** Sellers inflate profit by claiming excessive add-backs (owner salary, personal expenses, one-time costs). For each add-back, ask: "What is the true cost to replace this function?" Two distinct cases — do not blur them: (a) **Family members or staff at market rate doing real jobs are not add-backs** — if the spouse runs the office at a market salary, you must re-hire that role, so the add-back nets to zero. (b) **The owner's own role:** SDE by convention adds back ONE full-time owner's compensation, and the low owner-operator multiple is what prices in the fact that the buyer works that job. If you will NOT work the owner's role yourself, subtract a market-rate manager salary from SDE (which effectively converts it toward EBITDA) before applying a multiple. Apply the discount in ONE place — either the replacement-salary subtraction or the low multiple — never claim credit twice, and never skip it entirely. The adjusted profit is your valuation base. This step is critical: accepting inflated add-backs at a 3x multiple can mean overpaying by several hundred thousand dollars (roughly 20-40 percent on typical deals — a working estimate this system uses, not a Frasier statistic).
